...
Encapsulates all logic for validating workspace boundaries, creating MoveIt
...
class CartesianActionClient:
    ...
    # Default workspace boundaries in robot_base_link frame
    ...
    def check_workspace(self, x: float, y: float, z: float) -> Tuple[bool, str]:
        ...
        if not (min_z <= z <= max_z):
            violations.append(
                f"z={z:.4f} outside [{min_z:.3f}, {max_z:.3f}] "
                f"({'> max' if z > max_z else '< min'})"
            )
    ...
    def move_to_position(
        ...
        Args:
            ...
            duration_sec (float): Motion execution duration in seconds.
            feedback_callback (Callable, optional): Custom callback for feedback messages.
            result_callback (Callable, optional): Custom callback when goal finishes.

        Returns:
            bool: True if goal was validated and sent, False otherwise.
        """
        ...
        def internal_feedback(feedback_msg):
            ...
            self._logger.info(
                f"Trajectory progress: current position = ({pos.x:.4f}, {pos.y:.4f}, {pos.z:.4f})"
            )
        ...
        def internal_goal_response(future):
            ...
            def internal_result(res_f):
                ...
                if status == 4:  # STATUS_SUCCEEDED
                    ...
                    self._logger.error(f"Cartesian trajectory motion ABORTED: {res.result.msg}")
                else:
